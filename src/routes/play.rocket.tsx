  ...
  return (
    <div className="max-w-4xl mx-auto space-y-4">
      <h1 className="font-display text-3xl font-bold flex items-center gap-2">
        <Rocket className="text-amber-400" /> Rocket
      </h1>
      <CasinoFrame title="Set your target" subtitle="Cash out before the crash" icon={<Rocket className="size-6 text-amber-400" />}>
        <div className="grid md:grid-cols-[1fr_1.2fr] gap-6">
          <div className="space-y-4">
            ...
            <div>
              <Label className="text-amber-100">Target multiplier (1.01x – 100x)</Label>
              ...
              <div className="mt-2 flex gap-2 flex-wrap">
                {[1.25, 1.5, 2, 3, 5, 10].map((v) => (
                  <button key={v} disabled={busy} onClick={() => setTarget(v)}
                    className="px-2 py-1 rounded bg-amber-400/10 text-amber-100 text-xs hover:bg-amber-400/20 disabled:opacity-50">{v}x</button>
                ))}
              </div>
              <p className="text-[11px] text-amber-100/60 mt-2">
                Potential payout: <b className="text-amber-200">{fmt(Math.floor(bet * target))}</b> DICE
              </p>
            </div>
          ...
          >
            ...
            >
              <Rocket className="size-14 text-amber-300 drop-shadow-[0_0_20px_rgba(252,211,77,0.7)]" />
              {busy && (
                <motion.div
                  className="absolute left-1/2 -translate-x-1/2 -bottom-6 w-3 h-8 rounded-full bg-gradient-to-t from-red-500 via-orange-400 to-yellow-200"
            ...
            <div className="absolute inset-x-0 top-4 text-center">
              <div className={`font-display font-black text-5xl md:text-6xl ${result ? (result.won ? "text-emerald-300" : "text-red-400") : "text-amber-100"}`}>
                {live.toFixed(2)}x
              </div>
              <div className="text-[11px] uppercase tracking-widest text-amber-200/60 mt-1">
