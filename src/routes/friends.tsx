...
import { Search, UserPlus, Check, X, Users, Circle } from "lucide-react";
...
function presenceLabel(lastSeen?: string | null) {
  if (!lastSeen) return { label: "Offline", className: "text-muted-foreground" };
  const age = Date.now() - new Date(lastSeen).getTime();
  if (age <= 2 * 60_000) return { label: "Online", className: "text-emerald-400" };
  if (age <= 15 * 60_000) return { label: "Recently active", className: "text-amber-300" };
...
function Friends() {
  ...
  const search = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: rels } = await supabase.from("friendships").select("*").or(`and(requester_id.eq.${user.id},addressee_id.in.(${ids.join(",")})),and(addressee_id.eq.${user.id},requester_id.in.(${ids.join(",")}))`);
  ...
  return (
    <div className="space-y-4">
      <PageHeader icon={Users} title="Friends" subtitle={`${(friends.data ?? []).length} friend${(friends.data ?? []).length === 1 ? "" : "s"} · search anyone by @username`} accent="emerald" />
          ...
          <div className="mt-2 space-y-1">
            {(search.data ?? []).map((p: any) => (
              <div key={p.id} className="flex items-center justify-between rounded-md hover:bg-white/5 p-2">
                ...
                {p._rel === "friends" ? <span className="text-xs text-emerald-400 px-2">✓ Friends</span>
                  : p._rel === "sent" ? <span className="text-xs text-muted-foreground px-2">Requested</span>
                    : p._rel === "incoming" ? <span className="text-xs text-amber-400 px-2">Wants to be friends</span>
                      : p._rel === "blocked" ? <span className="text-xs text-destructive px-2">Blocked</span>
      ...
      {(pending.data ?? []).length > 0 && <Card className="glass p-5"><h2 className="font-display font-semibold mb-3">Pending requests</h2><div className="space-y-2">{(pending.data ?? []).map((r: any) => <div key={r.id} className="flex items-center justify-between rounded-md bg-white/5 p-2"><div className="flex items-center gap-2"><Avatar className="size-7"><AvatarImage src={r.profiles?.avatar_url} /><AvatarFallback>{r.profiles?.display_name?.[0]}</AvatarFallback></Avatar><span className="text-sm">{r.profiles?.display_name}</span></div><div className="flex gap-1"><Button size="sm" onClick={() => respond(r.id, true)}><Check className="size-4" /></Button><Button size="sm" variant="outline" onClick={() => respond(r.id, false)}><X className="size-4" /></Button></div></div>)}</div></Card>}

      <Card className="glass p-5">
        <h2 className="font-display font-semibold mb-3">Your friends</h2>
        {(friends.data ?? []).length === 0 ? <EmptyState icon={Users} title="No friends yet" description="Search above to find people on DICE." /> : (
          <div className="grid gap-2 md:grid-cols-2">{(friends.data ?? []).map((p: any) => {
            const status = presenceLabel(p.last_seen_at);
            return <Link key={p.id} to="/u/$username" params={{ username: p.username }} className="flex items-center gap-3 rounded-md bg-white/5 p-3 hover:bg-white/10"><div className="relative"><Avatar className="size-9"><AvatarImage src={p.avatar_url ?? undefined} /><AvatarFallback>{p.display_name[0]}</AvatarFallback></Avatar><Circle className={`absolute -bottom-0.5 -right-0.5 size-3 fill-current ${status.className} ring-2 ring-background`} /></div><div className="min-w-0"><div className="text-sm font-medium truncate">{p.display_name}</div><div className="text-xs text-muted-foreground">Lvl {p.level} · @{p.username}</div><div className={`text-[11px] mt-0.5 ${status.className}`}>{status.label}</div></div></Link>;
          })}</div>
        )}
      </Card>
    </div>
